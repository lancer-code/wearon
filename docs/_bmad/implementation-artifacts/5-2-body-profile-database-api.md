...
- [x] [AI-Review][HIGH] Unsafe type casting in `mapBodyProfile` without runtime validation: Direct `as number`/`as string` casts without validating actual types from database can cause runtime errors, silent data corruption, or security issues if schema changes or returns unexpected values. [packages/api/src/routers/body-profile.ts:29-39] **FIXED 2026-02-13**: Added `bodyProfileDbSchema` Zod schema for runtime validation; `mapBodyProfile` now validates types with `.parse()` before mapping.
- [x] [AI-Review][MEDIUM] No validation on measurement value ranges: `z.number().positive()` allows unrealistic values like `estChestCm: 999999`, breaking size-rec algorithms, enabling data poisoning attacks, and causing storage issues. [packages/api/src/routers/body-profile.ts:11-14] **FIXED 2026-02-13**: Added realistic measurement ranges (30-200cm for body measurements, 30-300kg for weight) using shared `measurementSchema`.
- [x] [AI-Review][MEDIUM] No rate limiting on profile mutation endpoints: Malicious/compromised users can spam `saveProfile`/`updateFromSizeRec` thousands of times causing database DoS, excessive writes, and storage/audit log bloat. [packages/api/src/routers/body-profile.ts:153] **FIXED 2026-02-13**: Implemented per-user rate limiter (10 updates/minute) with in-memory cache and automatic cleanup; throws `TOO_MANY_REQUESTS` error when exceeded.
- [x] [AI-Review][LOW] Inconsistent `heightCm` validation between endpoints: Validates `min(100).max(250)` but doesn't match Story 5.1's precision constraint (max 1 decimal place), causing validation inconsistency and potential for values to pass here but fail in size-rec. [packages/api/src/routers/body-profile.ts:6] **FIXED 2026-02-13**: Extracted shared `heightCmSchema` with same precision validation as Story 5.1 (max 1 decimal using `Number.isInteger(val * 10)`).
