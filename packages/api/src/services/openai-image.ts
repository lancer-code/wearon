...
async function resizeImageForCost(buffer: Buffer, imageName: string): Promise<Buffer> {
  ...
  if (!width || !height) {
    ...
    logger.info(`[Tokens] ${imageName}: unknown dimensions, assuming ~765 tokens`)
    return buffer
  }
  ...
  if (longestSide <= MAX_IMAGE_DIMENSION) {
    ...
    logger.info(`[Tokens] ${imageName}: ${width}x${height} = ${tokens} tokens (no resize needed)`)
  ...
  logger.info(`[Tokens] ${imageName}: ${width}x${height} (${originalTokens} tokens) → ${newWidth}x${newHeight} (${newTokens} tokens) - saved ${originalTokens - newTokens} tokens, ${reduction}% smaller file`)
...
export async function generateWithOpenAI(options: OpenAIImageOptions): Promise<OpenAIImageResponse> {
  ...
  logger.info(`[Cost] Estimated: ${imageBuffers.length} images × ~${estimatedTokensPerImage} tokens = ${totalInputTokens} input tokens`)
  logger.info(`[Cost] Input: $${inputCost.toFixed(4)} + Output: $${outputCost.toFixed(2)} = Total: $${totalCost.toFixed(4)}`)
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      logger.info(`[OpenAI Image] Attempt ${attempt}/${maxRetries}`)
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (status === 400 && errorCode === 'moderation_blocked') {
          logger.info('[OpenAI Image] Content moderation blocked request')
        ...
        if (status && status >= 500 && attempt < maxRetries) {
          logger.info(`[OpenAI Image] Server error ${status}, retrying in ${attempt * 2}s...`)
