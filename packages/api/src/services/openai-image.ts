...
/**
 * Alternative: Generate using collage image (like Grok approach)
 * This sends the stitched collage as a single image with instructions
 */
export async function generateFromCollage(options: {
  collageUrl: string
  prompt?: string
  quality?: 'low' | 'medium' | 'high'
  size?: '1024x1024' | '1024x1536' | '1536x1024'
}): Promise<OpenAIImageResponse> {
  const { collageUrl, prompt, quality = 'medium', size = '1024x1536' } = options
  const apiKey = getApiKey()

  console.log('[OpenAI Image] Generating from collage...')

  const collageBuffer = await downloadImage(collageUrl)

  const editPrompt =
    prompt ||
    `This is a 3-column virtual try-on reference collage:
...
- Output ONE portrait (3:4 ratio), NOT a collage`
  ...
  formData.append('size', size)
  formData.append('n', '1')
  formData.append('image', collageBuffer, {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`[OpenAI Image] Collage attempt ${attempt}/${maxRetries}`)
      ...
      console.log('[OpenAI Image] Collage generation success!')
      return {
        imageUrl: generatedUrl,
        model: 'gpt-image-1.5',
        created: response.data.created || Date.now(),
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (status && status >= 500 && attempt < maxRetries) {
          console.log(`[OpenAI Image] Server error ${status}, retrying...`)
          lastError = new OpenAIImageError(`API error: ${error.message}`, status, error.response?.data)
          await new Promise((r) => setTimeout(r, attempt * 2000))
          continue
        }
