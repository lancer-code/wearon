...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'user_body_profiles'
      AND policyname = 'Users can view own body profile'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'user_body_profiles'
      AND policyname = 'Users can insert own body profile'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'user_body_profiles'
      AND policyname = 'Users can update own body profile'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'update_user_body_profiles_updated_at'
  ) THEN
